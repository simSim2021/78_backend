...
// {title: "", text: ""}
router.post("/", (req, res) => {
  const { title, text } = req.body;//получаем данные из тела запроса от пользователя
...
// PATCH /posts/:id (редактирование имеющегося поста)
...
router.delete("/:id", (req, res) => {
  ...
  res.status(200).json(post);//возвращаем удаленный элемент
});
